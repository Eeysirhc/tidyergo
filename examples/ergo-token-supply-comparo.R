...
#####
# CREATE FUNCTION
comparo <- function(token_id, category_name){
...
df <- bind_rows(migoreng, comet, erdoge, neta, ergopad, paideia,
               egio, ergolend, thz, kushti, valleydao)

# CALCULATE % DISTRO
df_clean <- df %>%
  mutate(pct_p2pks = in_p2pks / emitted,
         pct_contracts = in_contracts / emitted,
         pct_burned = burned / emitted)

# PLOT GRAPH
df_clean %>%
  ggplot(aes(pct_p2pks, pct_contracts, label = name)) +
  geom_text_repel(aes(color = category), size = 5) +
  geom_point(aes(color = category, size = pct_burned), alpha = 0.7) +
  geom_abline(lty = 2) +
  labs(color = "Category",
       size = "% Burned",
